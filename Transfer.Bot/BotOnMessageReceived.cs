    ...
    public static class BotOnMessageReceived
    {
        public static async Task OnMessageReceived(this ITelegramBotClient bot, Message message, IUnitOfWork unitOfWork, ILogger logger = null, IMailModule mailModule = null)
        {
            ...
            if(user == null)
            {
                ...
                {
                    await mailModule.SendEmailPlainTextAsync($"<h2>Приветствуем вас</h2></br></br>Поступил запрос на подключение к телеграм боту.</br></br> Чтобы подтвердить перейдите по ссылке</br></br> <a href=\"https://nexttripto.ru/TgAccept/{ruser.Id}\" target=\"self\">перейти</a></br></br>При переходе по ссылке Вы подтверждаете свое согласие с <a href=\"https://nexttripto.ru/static/4ks6wz\" target=\"blank\">условиями</a> использования платформы, а так же с <a href=\"https://nexttripto.ru/static/sd9dzh\" target=\"blank\">договором</a> на оказание услуг по предоставлению доступа к пдатформе nexttripto.ru", "Запрос на подтверждение использования телеграм бота", ruser.Email);
